...
namespace wl.body.Test
{
    ...
    public class ReadModelBuilderTest
    {
        ...
        public void OnlyActivatedTasksAreCounted()
        {
            ...
            Assert.AreEqual(1, result.Length, "Expected one list item");
            Assert.AreEqual("1", result[0].Id, "List Id mismatch");
            Assert.AreEqual("Listen Name", result[0].Name, "List Name mismatch");
            Assert.AreEqual(2, result[0].NumberOfTasks, "Activated task mismatch.");
        }
    }
}
